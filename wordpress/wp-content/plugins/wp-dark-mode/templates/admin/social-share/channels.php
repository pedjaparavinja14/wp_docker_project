         ...
         <div class="mt-8">
             ...
             <div class="flex flex-col gap-2" dropzone="true">
                 <template x-for="channel in enabledChannels">
                     <div class="flex flex-col gap-2 w-80 relative" dropzone="true" draggable="true" @dragstart="state.draggingChannel = channel.id" @dragenter="handleDrag($event, channel.id)" @dragend="handleDrop($event, channel.id)" :class="`_channel-${channel.id}`">
                         ...
                         <div class="flex items-center gap-4 w-full cursor-pointer _social-share-container inline" tabindex="0">
                             ...
                             <div class="flex items-center bg-slate-200 _channel-name">
                                 ...
                                 <div class="flex w-28 items-center gap-2 relative">
                                     <div class="text-sm h-full w-full pl-3 font-medium flex items-center focus:outline-none transition duration-100 focus:ring focus:ring-blue-400 h-8" x-text="channel.name" contenteditable="true" @input="channel.name = $event.target.innerText"></div>
                                     <svg xmlns="http://www.w3.org/2000/svg" class="fill-current w-3 absolute right-2.5 top-1/2 -translate-y-1/2" viewBox="0 0 16 16" @click.prevent="editableChannel($event)">
                                         <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z" />
                                     </svg>
                                 </div>
                             </div>

                             <span @click.prevent="channel.visibility.mobile = !channel.visibility.mobile" class="bg-slate-200 h-10 w-11 flex items-center justify-center text-base transition duration-100" :class="channel.visibility.mobile ? ['text-blue-500'] : ['text-slate-300']" x-html="constant.devices.mobile.icon"></span>

                             <span @click.prevent="channel.visibility.desktop = !channel.visibility.desktop" class="bg-slate-200 h-10 w-11 flex items-center justify-center text-base transition duration-100" :class="channel.visibility.desktop ? ['text-blue-500'] : ['text-slate-300']" x-html="constant.devices.desktop.icon"></span>
